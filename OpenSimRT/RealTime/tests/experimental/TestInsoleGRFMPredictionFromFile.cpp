...
void run() {
    //TODO: remove stuff I don't need!!
    // subject data
    ...
    InsoleDetector::Parameters detectorParameters;
    //TODO: HEN???? Do I need this guy?
    //TODO: Remove Unused params!!!
    //detectorParameters.insoleDataFromFile = "some_file.csv_or_sto";
    ...
    auto detector = InsoleDetector(model, detectorParameters);

    // grfm prediction
    GRFMPredictionInsole::Parameters grfmParameters;
    grfmParameters.method = GRFMPredictionInsole::selectMethod(grfmMethod);
    grfmParameters.insoleDataFromFile = "some_file.csv_or_sto";
    ...
    GRFMPredictionInsole grfm(model, grfmParameters, &detector);
    ...
    for (int k = 0; k < qTable.getNumRows() * simulationLoops; k++) {
        ...
        detector.updDetector({ikFiltered.t, q, qDot, qDDot}); // here maybe I want to also get the value from the insole somehow?
